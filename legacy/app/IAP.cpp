...
int IAP::erase( uint32_t sector )
{
    int err = prepare(sector, sector);
    ...
    IAP_command[ 2 ]    = (unsigned int)sector;    //  End Sector Number (should be greater than or equal to start sector number)
...
{
    uint32_t sector_start = find_sector(target_addr);
    uint32_t sector_end = find_sector(target_addr+size-1);
...
int IAP::get_sector_size( uint32_t addr )
{
    if(find_sector(addr)>=0x10) {
        return 0x8000;
    } else {
        return 0x1000;
    }
