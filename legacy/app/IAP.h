...
public:
     ...
     */
    int write( uint32_t *source_addr, uint32_t target_addr, int size );
     ...
     */
    int compare( uint32_t *source_addr, uint32_t *target_addr, int size );
    ...
    uint32_t *reserved_flash_area_start( void );
    ...
    int find_sector( uint32_t addr );

    int get_sector_size( uint32_t addr );
     ...
     *  Remark: The top 64 bytes of the EEPROM memory are reserved and cannot be written to.
     */
    int read_eeprom( uint32_t *source_addr, uint32_t *target_addr, int size );
